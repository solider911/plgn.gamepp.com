            ...
            if ($returnUserInfo==true){
                ...
                if ($utype == '1'){
                    ...
                    $add_token = Db::table('ys_login_wb')
                        ...
                        ->setField('user_wb_token',hash("sha1",$user_info['user_wb_openid'].time()));
                    //获取token
                    $user_data = Db::table('ys_login_wb')
                        ->where('user_wb_id','=',$user_info['user_wb_id'])
                        ->find();
                ...
                if($utype == '1'){
                    ...
                    $add_token = Db::table('ys_login_wb')
                        ->where('user_wb_id','=',$user_info['user_wb_id'])
                        ->setField('user_wb_token',hash("sha1",$userInfo['user_wb_openid'].time()));
                    //获取token
                    $user_data = Db::table('ys_login_wb')
                        ->where('user_wb_id','=',$user_info['user_wb_id'])
                        ->find();
